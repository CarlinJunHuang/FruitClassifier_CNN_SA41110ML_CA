...
# ==========================================
TEAM_MEMBER_NAME = "Raj"       # Name of the team member running the experiment
TRIAL_PARAM_NAME = "Neuron"  # Parameter being tested (e.g., "lr_0.01", "epoch_20")
NUM_EPOCHS = 10
BATCH_SIZE = 32
IMAGE_SIZE = 64
LEARNING_RATE = 0.001
...
class SimpleCNN(nn.Module):
    def __init__(
            self,
            img_channels = 3,
            first_out_channels = 256,
            out_features = 1024,
            final_out_features = 4,
            ):
        super(SimpleCNN, self).__init__()
        # Input: 3 x 32 x 32
        self.conv1 = nn.Conv2d(img_channels, first_out_channels, kernel_size=3, padding=1)
        self.conv2 = nn.Conv2d(first_out_channels, first_out_channels*2, kernel_size=3, padding=1)
        self.pool = nn.MaxPool2d(kernel_size=2, stride=2)
        
        self.fc_input_size = (first_out_channels*2) * (IMAGE_SIZE//4) * (IMAGE_SIZE//4)
        
        self.fc1 = nn.Linear(in_features=self.fc_input_size, out_features=out_features)
        self.fc2 = nn.Linear(in_features=out_features, out_features=final_out_features)
        self.relu = nn.ReLU()
