...
# ==========================================
TEAM_MEMBER_NAME = "Maha"       # Name of the team member running the experiment
TRIAL_PARAM_NAME = "New_images"  # Parameter being tested (e.g., "lr_0.01", "epoch_20")
NUM_EPOCHS = 10
BATCH_SIZE = 32
IMAGE_SIZE = 64
LEARNING_RATE = 0.001
...
class SimpleCNN(nn.Module):
    def __init__(self):
        ...
        self.fc_input_size = self.calculate_fc_input_size(IMAGE_SIZE)
    ...
    def calculate_output_size(self, input_size, kernel_size, stride=1, padding=0):
        """Calculate output size after a convolution or pooling layer"""
        return (input_size + 2 * padding - kernel_size) // stride + 1
    
    def calculate_fc_input_size(self, image_size):
        """Calculate the flattened size after all conv and pool layers"""
        size = image_size
        
        # After conv1 (kernel=3, stride=1, padding=1)
        size = self.calculate_output_size(size, kernel_size=3, stride=1, padding=1)
        
        # After pool1 (kernel=2, stride=2, padding=0)
        size = self.calculate_output_size(size, kernel_size=3, stride=1, padding=0)
        
        # After conv2 (kernel=3, stride=1, padding=1)
        size = self.calculate_output_size(size, kernel_size=3, stride=1, padding=1)
        
        # After pool2 (kernel=2, stride=2, padding=0)
        size = self.calculate_output_size(size, kernel_size=3, stride=1, padding=0)
        
        # Final flattened size: channels * height * width
        return 32 * size * size
